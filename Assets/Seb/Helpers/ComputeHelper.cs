...
// This class contains some helper functions to make life a little easier working with compute shaders (Very work-in-progress!)
	...
	public static class ComputeHelper
	{
		...
		public static void Dispatch<T>(ComputeShader cs, int numIterationsX, int numIterationsY, int numIterationsZ, T kernel) where T : Enum
		{
			Dispatch(cs, numIterationsX, numIterationsY, numIterationsZ, Convert.ToInt32(kernel));
		}

		public static void Dispatch<T>(ComputeShader cs, int numIterationsX, T kernel) where T : Enum
		{
			Dispatch(cs, numIterationsX, 1, 1, Convert.ToInt32(kernel));
		}

		public static void Dispatch<T>(ComputeShader cs, int numIterationsX, int numIterationsY, T kernel) where T : Enum
		{
			Dispatch(cs, numIterationsX, numIterationsY, 1, Convert.ToInt32(kernel));
		}
		...
		{
			int stride = GetStride<T>();
			int desiredCount = Mathf.Max(1, data.Count);
			bool createNewBuffer = buffer == null || !buffer.IsValid() || buffer.count < desiredCount || buffer.stride != stride;

			if (createNewBuffer)
			{
				Release(buffer);
				buffer = new ComputeBuffer(desiredCount, stride);
			}
		...
		// Create new structured buffer if current buffer doesn't match requirements.
		// Note: this version will not create a new buffer if the current one is larger than required (i.e. it won't shrink it)
		public static void CreateStructuredBuffer_DontShrink<T>(ref ComputeBuffer buffer, T[] data) where T : struct
		{
			int stride = GetStride<T>();
			int desiredCount = Mathf.Max(1, data.Length);
			bool createNewBuffer = buffer == null || !buffer.IsValid() || buffer.count < desiredCount || buffer.stride != stride;
		...
		public static void SetBuffer<T>(ComputeShader compute, ComputeBuffer buffer, string id, params T[] kernels) where T : Enum
		{
			for (int i = 0; i < kernels.Length; i++)
			{
				compute.SetBuffer(Convert.ToInt32(kernels[i]), id, buffer);
			}
		}
		...
		{
			CreateRenderTexture(ref texture, width, height, defaultFilterMode, RGBA_SFloat);
		}

		public static RenderTexture CreateRenderTexture(int width, int height)
		{
			return CreateRenderTexture(width, height, defaultFilterMode, RGBA_SFloat);
		}
		...
		public static void SetTexture<T>(ComputeShader compute, Texture texture, string name, params T[] kernels) where T : Enum
		{
			for (int i = 0; i < kernels.Length; i++)
			{
				compute.SetTexture(System.Convert.ToInt32(kernels[i]), name, texture);
			}
		}
